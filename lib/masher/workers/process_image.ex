defmodule Masher.Workers.ProcessImage do
  ...
  @impl true
  def perform(%Oban.Job{args: %{"image_key" => key, "variants" => variants}}) do
    Logger.info("Processing image #{key}")

    {:ok, binary} = Masher.S3.download(key)
    ...
    variant_keys =
      for [name, max_dim, quality] <- variants do
        ...
        Masher.S3.upload(variant_key, webp_binary, "image/webp")

        {name, variant_key}
      end

    Masher.S3.delete(key)

    Phoenix.PubSub.broadcast(:masher_pubsub, "image_results",
      {:image_mashed, key, variant_keys})

    Logger.info("Completed processing #{key}")
